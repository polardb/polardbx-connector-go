...
func runTestsWithMultiStatement(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	...
	var db *sql.DB
	if _, err := ParsePolarDBXDSN(dsn); err == nil {
...
func runTests(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	...
	var db2 *sql.DB
	if _, err := ParsePolarDBXDSN(dsn2); err == nil {
...
func runTestsParallel(t *testing.T, dsn string, tests ...func(dbt *DBTest, tableName string)) {
	...
	for _, test := range tests {
		...
		t.Run("default", func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("error connecting: %s, dsn: %s", err.Error(), dsn)
			}
			t.Cleanup(func() {
...
//	cfg, err := ParsePolarDBXDSN(fmt.Sprintf("%s:%s@%s/%s?timeout=30s", user, pass, netAddr, "_"))
//	if err != nil {
...
func TestMySQLEmptyPassword(t *testing.T) {
	...
	dsn := fmt.Sprintf("%s:%s@tcp(%s)/?charset=utf8mb4&parseTime=True&enableLog=%s&enableProbeLog=%s",
		*user, "", *addr, *enableLog, *enableProbeLog)
	db, err := sql.Open(driverNameTest, dsn)
	if err == nil {
		defer db.Close()
		err = db.Ping()
	...
	if *passwd == "" {
		...
		if !strings.HasPrefix(err.Error(), "Error 1045") {
			t.Fatal("driver: " + driverNameTest + ", dsn: " + dsn + ", error: " + err.Error())
		}
	}
}
...
var (
	_ driver.ConnBeginTx        = &polardbxConn{}
	_ driver.ConnPrepareContext = &polardbxConn{}
	_ driver.ExecerContext      = &polardbxConn{}
	_ driver.Pinger             = &polardbxConn{}
	_ driver.QueryerContext     = &polardbxConn{}
)
...
var _ driver.DriverContext = &PolarDBxDriver{}
